...
type CacheManager struct {
	...
	downBandwidthManager *bandwidthManager
}
...
func CreateCacheManager(prefix string, upstreamHost string, cacheCap int64, cacheDuration int64) (CacheManager, error) {
	manager := CacheManager{
		...
		downBandwidthManager: CreateBandwidthManager(),
	}
...
func (manager CacheManager) fetchItem(key KeyType, req *http.Request, cacheTask CacheTask) {
	...
	manager.downBandwidthManager.Update(cacheSize)
	g_downBandwidthManager.Update(cacheSize)
...
func (manager CacheManager) fetchFromUpstream(req *http.Request, key KeyType) *CacheItem {
	...
	logMessage("fetching from upstream %s", upstreamUrl)
	...
	logMessage("fetched from upstream %s. contentType=%s, contentLength=%d", upstreamUrl, contentType, bodyLen)

	atomic.AddInt64(&g_stats.BytesReadFromUpstream, int64(bodyLen))
	return &item
}
