...
type LogSetting struct {
	Filename   string // "/var/log/gulfire/access/gulfire-access.log"
...
func usage() {
	fmt.Printf("Gulfire is a live HLS cache server\n"+
...
func init() {
	...
	flag.StringVar(&Settings.id, "id", "", "A name for this service, hostname will be used if it's empty, it will be added to the HTTP response header")
	...
	flag.StringVar(&Settings.statsRequestPath, "statsRequestPath", "/static_stats", "Path to page with statistics")
	flag.StringVar(&Settings.statsJsonRequestPath, "statsJsonRequestPath", "/static_statsjson", "Path to page with statistics")
	...
	flag.StringVar(&Settings.CollectorName, "collectorName", "", "A name for letting the RTCollector know where the data come from, id will be used if it is missing")
	flag.StringVar(&Settings.CollectorServer, "collectorServer", "", "Path to connect to the RTCollector")

	flag.BoolVar(&Settings.debug, "debug", false, "output debug information when it's true")
}
